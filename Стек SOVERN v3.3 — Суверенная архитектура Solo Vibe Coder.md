...
│    architect.md (Ramp PM)  ·  ingest_email  ·  desops (design.md)    │
...
---
## 24. DesOps Hub — Design as Executable Spec (v3.3 Integration)
**Статус:** Deployed June 2026. Centralized Governance / Decentralized Execution.

**Конвейер:**
1. **Master Tokens:** `GLOBAL_DESIGN.md` в ядре NAUTILUS — единый источник истины для цветов, шрифтов и сеток.
2. **Local Specs:** Каждый проект в `Efforts` имеет свой `DESIGN.md`, наследующий (extends) глобальные токены.
3. **Pencil Precision:** Векторные макеты (`.pen`) хранятся локально в папках проектов (`design/`), предотвращая "свалку" в ядре.
4. **Automation:** `Atlas/Scripts/DesOps/` содержит глобальные скрипты линтинга и экспорта (Tailwind/CSS).

**Принцип:** Дизайн — это не картинка, а валидируемый код. Любое изменение в глобальных токенах каскадно обновляет всю экосистему через автоматизированный экспорт.
