# Стек Agentic AI V3.3 — Суверенная Архитектура Solo Vibe Coder
...
| **v3.3** | **4** | **Ollama-нативный Hermes; Compound Engineering plugin; DeepVista skill schema; PARA фаза-0 памяти; Ramp PM skill; NotebookLM as pipeline; Apify; Substack для digital-goods** | **WSL2 для Hermes; custom evo-loop; 1-фазная graphiti с нуля** |
---
## 1. Слоистая архитектура (one-page mental model)
```javascript
┌─────────────────────────────────────────────────────────────────────┐
│  HUMAN (Architect · PM · Operator · Reviewer)                       │
│  ↕ Telegram + Zed + Claude Code CLI                                 │
├─────────────────────────────────────────────────────────────────────┤
│  BOSS / ORCHESTRATION                                                │
│    Hermes Agent (ollama launch hermes) — skills, cron, gateways      │
├─────────────────────────────────────────────────────────────────────┤
│  SKILLS (DeepVista schema: type × execution, --dry-run)             │
...
│    └─ cloud rotating pool: Cerebras → Groq → NIM → OpenRouter        │
│       optional: Anthropic API pay-as-you-go via Aider                │
├─────────────────────────────────────────────────────────────────────┤
│  MEMORY                                                              │
│    Phase 0 (Felix/PARA):  Obsidian ~/life/ + daily.md + tacit.md     │
...
---
## 4. Boss/Orchestration — Hermes Agent (Ollama-нативный)
**Установка (новое в v3.3):**
```javascript
ollama launch hermes

```
Это всё. Ollama 0.21 (письмо `hello@ollama.com`, 18 апр 2026) и `docs.ollama.com/integrations/hermes` дают нативную поддержку: skills from experience, persistent knowledge, search of past conversations. **Долой WSL2-обёртку** из v3.1.
**Ответственность Hermes.** Telegram-relay primary, cron triggers, skills lifecycle, sub-agent dispatch, `weekly-digest` skill (см. §11). **НЕ coding harness** — это Loop 2 verdict, остаётся.
...
---
## 7. Memory — двухступенчатая (PARA fast path → graphiti slow path)
**Главное обновление v3.3 (P4):** не строить graphiti с нуля. Сначала фаза 0 — PARA на Markdown.
### Фаза 0 — Felix/PARA stack (~30 минут setup)
...
litellm_settings:
  fallbacks: [{"fast-pool": ["local-fallback"]}]    # CRITICAL — issue #26015
...
---
## 14. Voice — Handy primary (P-validation Loop 4)
...
---
## 18. Phased rollout (revised v3.3)
...
# Felix/PARA fast path (P4) — критичный shortcut
# create ~/life/{projects,areas,resources,archives,daily,tacit}/
...
---
## 21. Anthropic-bias accounting (final)
